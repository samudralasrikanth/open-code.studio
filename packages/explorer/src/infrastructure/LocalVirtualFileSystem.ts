...
import { ExplorerIgnoreService } from "../domain/ExplorerIgnoreService.js";
...
export class LocalVirtualFileSystem implements IVirtualFileSystem {
  private ignoreService: ExplorerIgnoreService;

  constructor(ignoreService = new ExplorerIgnoreService()) {
  ...
  public async readDirectory(uri: WorkspaceUri): Promise<[string, FileType][]> {
    ...
    return entries
      .filter((entry) => !this.ignoreService.shouldIgnore(entry.name))
      .map((entry) => {
        let type = FileType.Unknown;
