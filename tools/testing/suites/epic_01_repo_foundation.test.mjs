...
import fs from "fs";
import {
  ElectronE2ETestHarness,
  projectRoot,
  studioRoot
} from "../helpers/electron_e2e_helper.mjs";
...
export async function run() {
  ...
  try {
    ...
    harness.assert(
      ...
      "Lockfile (pnpm-lock.yaml) exists in workspace root"
    );

    const mainDist = path.join(studioRoot, "dist/main/index.js");
    harness.assert(
      fs.existsSync(mainDist),
      "Main process production bundle exists (dist/main/index.js)"
    );

    const preloadDist = path.join(studioRoot, "dist/preload/index.js");
    harness.assert(
      fs.existsSync(preloadDist),
      "Preload production bundle exists (dist/preload/index.js)"
    );

    const rendererDist = path.join(studioRoot, "dist/renderer/index.html");
    harness.assert(
      fs.existsSync(rendererDist),
      "Renderer production bundle exists (dist/renderer/index.html)"
    );
    ...
    harness.assert(
      ...
      `Window bounds valid (${bounds.width}x${bounds.height})`
    );
